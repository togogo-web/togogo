...
const webpackConfig = merge(baseWebpackConfig, {
	...
	plugins: [
		...
		new webpack.optimize.CommonsChunkPlugin({
			...
			minChunks: function (module) {
				...
				for(var plugin in commonPlugins){
					result = result && module.resource.indexOf(plugin) === -1
				}
				return result
			}
		}),
		...
		new CopyWebpackPlugin(utils.get('copyAssets'))
	].concat(utils.get('proHtmlPlugins')).concat(utils.get('proCommonPlugins'))
})
